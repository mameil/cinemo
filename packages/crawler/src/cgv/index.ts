 ...
 * CGV 크롤러 실행 엔트리
 *
 *   pnpm --filter @cinemo/crawler cgv              # 전체 수집 + DB 적재
 *   pnpm --filter @cinemo/crawler cgv -- --dry     # 수집 결과만 출력 (DB 미적재)
 *   pnpm --filter @cinemo/crawler cgv -- --max=3   # 최대 3건만
 */

import { collectCgv } from "./collect";
...
async function main() {
  ...
  console.log(`=== CGV 수집 시작 ${dry ? "(dry-run: DB 미적재)" : ""} ===`);
  const collected = await collectCgv({ maxEvents });
  console.log(`특전 이벤트 ${collected.length}건 수집\n`);
  ...
  const stats = await ingest("CGV", collected);
  console.log("=== 적재 완료 ===");
  console.log(stats);
}
